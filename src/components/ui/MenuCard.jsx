...
const MenuCard = ({ item, onAdd }) => {
  ...
  return (
    <div className={`group relative bg-nostro-charcoal rounded-3xl overflow-hidden border transition-all duration-500 flex flex-col h-full ${
      isSoldOut 
        ? 'border-white/5 opacity-70 grayscale' 
        : 'border-white/5 hover:border-nostro-orange/50 hover:shadow-2xl hover:shadow-nostro-orange/10'
    }`}>
      ...
      <div className="relative h-64 overflow-hidden shrink-0">
        ...
        />
        <div className="absolute inset-0 bg-gradient-to-t from-nostro-black/90 via-transparent to-transparent opacity-60"></div>
        ...
        <div className="absolute top-4 left-4 flex flex-col gap-2">
          ...
          {!isSoldOut && hasDeal && (
            <div className="px-3 py-1 bg-nostro-orange text-white text-xs font-bold uppercase rounded-full shadow-lg flex items-center gap-1 animate-pulse">
      ...
      <div className="p-6 flex flex-col flex-1 relative">
        <div className="flex justify-between items-start mb-2">
          <h3 className="text-xl font-serif font-bold text-white group-hover:text-nostro-orange transition-colors">
            {item.name}
          </h3>
          ...
          <div className="text-right ml-2">
            {hasDeal ? (
              <>
                ...
                <div className="text-lg font-bold text-nostro-orange">
                  KES {item.sale_price}
                </div>
              </>
            ) : (
              <span className="text-lg font-bold text-nostro-gold">
        ...
        <button 
          ...
          className={`w-full py-3 rounded-xl font-medium transition-all duration-300 flex items-center justify-center gap-2 mt-auto ${
            isSoldOut 
              ...
              : 'bg-white/5 hover:bg-nostro-orange text-white hover:text-white group-hover:translate-y-[-2px]'
          }`}
        >
