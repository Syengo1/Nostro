    ...
    <AnimatePresence>
      {isOpen && (
        <>
          ...
          <motion.div initial={{ x: '100%' }} animate={{ x: 0 }} exit={{ x: '100%' }} transition={{ type: "spring", stiffness: 300, damping: 30 }} className="fixed top-0 right-0 h-full w-full md:w-[450px] bg-notsro-charcoal border-l border-white/10 z-[70] flex flex-col shadow-2xl">
            <div className="p-6 border-b border-white/10 flex justify-between items-center bg-notsro-black z-10">
                ...
                <div className="space-y-8">
                  {cartItems.length === 0 ? (
                    ...
                    <div className="space-y-4">
                      {cartItems.map((item, index) => (
                        <motion.div layout initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} key={`${item.id}-${index}`} className="flex gap-4 p-4 bg-notsro-black rounded-xl border border-white/5">
                          <img src={item.image_url || item.image} alt={item.name} className="w-20 h-20 object-cover rounded-lg" />
                          <div className="flex-1 flex flex-col justify-center"><h4 className="font-serif font-bold text-white">{item.name}</h4><div className="text-notsro-orange font-bold text-sm">KES {item.price}</div></div>
                  ...
                  {cartItems.length > 0 && (
                    <div className="space-y-6">
                      ...
                      <div className="flex bg-notsro-black p-1 rounded-xl border border-white/10">
                        <button onClick={() => setFulfillmentType('delivery')} className={`flex-1 py-3 rounded-lg text-sm font-bold flex items-center justify-center gap-2 transition-all ${fulfillmentType === 'delivery' ? 'bg-notsro-orange text-white shadow-lg' : 'text-white/50 hover:text-white'}`}><MapPin className="w-4 h-4" /> Delivery</button>
                        <button onClick={() => setFulfillmentType('pickup')} className={`flex-1 py-3 rounded-lg text-sm font-bold flex items-center justify-center gap-2 transition-all ${fulfillmentType === 'pickup' ? 'bg-white text-notsro-black shadow-lg' : 'text-white/50 hover:text-white'}`}><Store className="w-4 h-4" /> Pickup</button>
                      </div>

                      <AnimatePresence mode='wait'>
                        {fulfillmentType === 'delivery' ? (
                          <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="space-y-4">
                            <div className="p-4 rounded-xl bg-notsro-black/50 border border-white/5 space-y-4">
                              <label className="text-xs uppercase text-white/50 tracking-widest font-bold">Where to?</label>
                              <button onClick={handleGetLocation} className="w-full py-3 border border-notsro-orange/30 bg-notsro-orange/10 text-notsro-orange rounded-xl flex items-center justify-center gap-2 hover:bg-notsro-orange/20 transition font-medium text-sm">{locationStatus === 'loading' ? <span className="w-4 h-4 border-2 border-current border-t-transparent rounded-full animate-spin"></span> : <Navigation className="w-4 h-4" />}{locationStatus === 'loading' ? "Locating..." : "Use Current Location"}</button>
                              <textarea placeholder="Or type address..." value={address} onChange={(e) => setAddress(e.target.value)} className="w-full bg-notsro-charcoal border border-white/10 rounded-xl p-3 text-white text-sm focus:border-notsro-orange focus:outline-none resize-none h-20"></textarea>
                            </div>
                            <div className="p-4 rounded-xl bg-notsro-black/50 border border-white/5 space-y-4">
                              <label className="text-xs uppercase text-white/50 tracking-widest font-bold">Who is it for?</label>
                              <div className="space-y-3">
                                <div className="relative"><User className="absolute left-3 top-3 w-4 h-4 text-white/30" /><input type="text" placeholder="Receiver Name" value={recipientName} onChange={(e) => setRecipientName(e.target.value)} className="w-full bg-notsro-charcoal border border-white/10 rounded-xl py-2 pl-10 pr-3 text-white text-sm focus:border-notsro-orange focus:outline-none" /></div>
                                <div className="relative"><Smartphone className="absolute left-3 top-3 w-4 h-4 text-white/30" /><input type="tel" placeholder="Receiver Phone" value={recipientPhone} onChange={(e) => setRecipientPhone(e.target.value)} className="w-full bg-notsro-charcoal border border-white/10 rounded-xl py-2 pl-10 pr-3 text-white text-sm focus:border-notsro-orange focus:outline-none" /></div>
                              </div>
                            </div>
                          </motion.div>
                        ) : (
                          <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="p-4 bg-notsro-black/50 rounded-xl border border-white/10 text-center"><p className="text-notsro-orange font-bold mb-1">Westlands Branch</p><p className="text-white/60 text-xs">Delta Towers, Ground Floor</p></motion.div>
                        )}
                      </AnimatePresence>
            ...
            {cartItems.length > 0 && checkoutState === 'idle' && (
              <div className="bg-notsro-black border-t border-white/10 p-6 space-y-4 z-20 shadow-[0_-10px_40px_rgba(0,0,0,0.5)]">
                <div className="flex gap-2">
                  <div className="relative flex-1"><Tag className="absolute left-3 top-2.5 w-4 h-4 text-white/30" /><input type="text" placeholder="PROMO CODE" value={couponCode} onChange={(e) => setCouponCode(e.target.value)} className="w-full bg-notsro-black border border-white/10 rounded-lg py-2 pl-9 pr-3 text-sm text-white uppercase focus:border-notsro-orange focus:outline-none" /></div>
                ...
                <div className="space-y-2">
                  ...
                  {fulfillmentType === 'delivery' && <div className="flex justify-between text-notsro-orange text-sm"><span>Delivery Fee</span><span>+ KES {DELIVERY_FEE}</span></div>}
